...
export const metadata: Metadata = {
    ...
    description: "Browse and download past papers by subject, board, class level, and year.",
}
...
export default async function PastPapersPage({ searchParams }: { searchParams: SearchParams }) {
    ...
    let dataQuery = supabase
        .from('past_papers')
        .select('id,subject,year,board,class_level,exam_shift,is_solved,created_at')
    ...
    return (
        <div className="container mx-auto px-4 py-8 space-y-8">
            <div className="rounded-3xl bg-primary-900 p-8 text-white">
                <h1 className="text-3xl font-black mb-2">Past Papers</h1>
                <p className="text-primary-100">Find papers by subject, board, class level, and year.</p>
            </div>

            <form className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                <div className="relative sm:col-span-2">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                    <Input name="q" defaultValue={q} placeholder="Search subject, board or class..." className="pl-9 h-11 bg-white" />
                </div>
                <Input name="board" defaultValue={board} placeholder="Board" className="h-11 bg-white" />
                <Input name="class_level" defaultValue={classLevel} placeholder="Class level" className="h-11 bg-white" />
                <Input name="year" defaultValue={year} placeholder="Year" inputMode="numeric" className="h-11 bg-white" />
                <Button type="submit" className="h-11 lg:col-start-5">Apply</Button>
            </form>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {(data || []).map((item) => (
                        <ListingCard
                            ...
                            type={`${item.year}`}
                            meta={item.is_solved ? 'Solved' : 'Unsolved'}
